...
public class BarbershopRepository(AppDbContext dbContext) : IBarbershopRepository
{
    ...
    public async Task<IReadOnlyCollection<Barbershop>?> GetByBarbershopOwnerIdAsync(Guid barbershopOwnerId,
    ...
    public async Task<IReadOnlyCollection<Barbershop>> ListByIdsAsync(IEnumerable<Guid> barbershopIds,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IReadOnlyCollection<Barbershop>> ListAsync(string? city = null,
        CancellationToken cancellationToken = default)
    {
        var query = dbContext.Barbershops.AsQueryable();

        if (!string.IsNullOrWhiteSpace(city)) query = query.Where(barbershop => barbershop.City == city);
