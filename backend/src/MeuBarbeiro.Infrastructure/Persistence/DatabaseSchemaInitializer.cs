...
public sealed class DatabaseSchemaInitializer
{
    ...
    public async Task EnsureSchemaAsync(AppDbContext dbContext, CancellationToken cancellationToken = default)
    {
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            """CREATE UNIQUE INDEX IF NOT EXISTS "IX_Users_Email" ON "Users" ("Email");""", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync(CreateClientsTableSql, cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync(
            """CREATE UNIQUE INDEX IF NOT EXISTS "IX_Clients_UserId" ON "Clients" ("UserId");""", cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync(CreateBarbersTableSql, cancellationToken);
        await dbContext.Database.ExecuteSqlRawAsync(
            """CREATE UNIQUE INDEX IF NOT EXISTS "IX_Barbers_UserId" ON "Barbers" ("UserId");""", cancellationToken);
        ...
        await dbContext.Database.ExecuteSqlRawAsync(
            """CREATE UNIQUE INDEX IF NOT EXISTS "IX_Reviews_AppointmentId" ON "Reviews" ("AppointmentId");""",
            cancellationToken);
    }
}