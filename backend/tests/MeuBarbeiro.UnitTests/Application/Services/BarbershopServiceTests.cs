...
public class BarbershopServiceTests
{
    ...
    [Fact]
    public async Task CreateBarbershop_DeveCriarBarbearia_QuandoTiverValoresValidos()
    {
        // Arrange
        var request = CreateBarbershopRequestDto();
        var barbershopOwnerId = Guid.NewGuid();

        _mockBarbershopRepository.Setup(b => b.AddAsync(It.IsAny<Barbershop>(), It.IsAny<CancellationToken>()));
        ...
        result.Data.Description.Should().BeEquivalentTo(request.Description);
    }
